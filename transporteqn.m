...
% DA FARE 
% fare delle funzioni per generalizzare 
% calcolo degli errori per tutto e confrontro tra gli errori 
% FARE CONFRONTO ERRORI 
...
for i = 1:length(t)
    for j = 1:length(x)
        if abs(x(j)+c*t(i))<(L/2)
        u(i,j) = U/2*(1+cos(2*pi*(x(j)+c*t(i))/L));
        elseif abs(x(j)+c*t(i))>(L/2)
        u(i,j) = 0;
        end 
    end
    figure(2)
...
if c > 0
    ...
    for i = 2:(length(t)-1)
        v(i,1)=v(i-1,1)+c*lambda*(v(i-1,2)-v(i-1,1)); % condizione al contorno 
        for j = 1:(length(x)-1)
            v(i,j)= v(i-1,j)+c*lambda*(v(i-1,j+1)-v(i-1,j));     
        end 
        figure(4)
    ...
    for i = 2:(length(t))
        v(i,N)=v(i-1,N)+c*lambda*(v(i-1,N)-v(i-1,N-1)); % condizione al contorno 
        for j = 2:(length(x))
            v(i,j)=v(i-1,j)+c*lambda*(v(i-1,j)-v(i-1,j-1));
        end 
        figure(4)
        plot(x,u(i,:),x,v(i,:),'LineWidth',1.1);
...
for i = 2:(length(x)-1)
        A(i,i+1) = -c*lambda*0.5;
end
for i = 1:(length(x)-1)
        A(i+1,i) = c*lambda*0.5;
end
...
if c > 0
    ...
    B(1,1)=0; 
    B(length(x),length(x))=1+c*lambda;
    B(length(x),length(x)-1)=-c*lambda;
end 
...
for i = 3:(length(t))
    % condizioni al contorno 
    if c > 0
        vLF(i,1)=vLF(i-1,1)+c*lambda*(vLF(i-1,2)-vLF(i-1,1)); 
    else 
        vLF(i,N)=vLF(i-1,N)+c*lambda*(vLF(i-1,N)-vLF(i-1,N-1));
    end 
     
    for j = 2:(length(x)-1)
        vLF(i,j)= vLF(i-2,j)+c*lambda*(vLF(i-1,j+1)-vLF(i-1,j-1));     
    end 
...
for i = 2:(length(t))
    % condizioni al contorno 
    if c > 0
        vLW(i,1)=vLW(i-1,1)+c*lambda*(vLW(i-1,2)-vLW(i-1,1)); 
    else 
        vLW(i,N)=vLW(i-1,N)+c*lambda*(vLW(i-1,N)-vLW(i-1,N-1));
    end 
     
    for j = 2:(length(x)-1)
        vLW(i,j)= vLW(i-1,j)+c*lambda*0.5*(vLW(i-1,j+1)-vLW(i-1,j-1))+c^2*lambda^2*0.5*(vLW(i-1,j+1)-2*vLW(i-1,j)+vLW(i-1,j-1));     
    end 
